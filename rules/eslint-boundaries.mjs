...
const GENERATED_ROOT = path.join(PROJECT_ROOT, contract.generatedRoot)
...
function isGeneratedFile(absolute) {
  const relative = path.relative(GENERATED_ROOT, absolute)
...
const capabilityRule = {
  meta: {
    ...
    messages: {
      ...
      neutralSurfaceDirection:
        'Runtime-neutral public surfaces may import only their capability domain or admitted shared/kernel code.',
    },
  },
  ...
  create(context) {
    ...
    const reportImport = (node, specifier) => {
      ...
      const targetIsGenerated = isGeneratedFile(targetPath)
      ...
      const sourceIsClient = sourceIsClientDirective || isClientLocation(sourceModule, sourceShared)
