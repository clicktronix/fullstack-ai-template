...
} from '@/shared/kernel/errors/action-error'
...
import { getStatusForCode } from '@/shared/server/api/response'
...
export const actionClient = createSafeActionClient({
  ...
  handleServerError(error) {
    ...
    // This is the inbound boundary for every Server Action built on actionClient or
    // authActionClient — capture unexpected errors here exactly once.
    // Already-coded errors thrown by capability policy or validation are expected, typed
    // outcomes — not incidents — so they are re-thrown/mapped without a Sentry capture.
