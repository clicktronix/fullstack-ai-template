...
function buildFallbackSuggestions(
  ...
  const firstPriorityItem = input.workItems.find((item) => item.isPriority)
  const unlabeledItems = input.workItems.filter((item) => item.labelIds.length === 0)
...
async function fetchAssistantSuggestionsFromApi(
  ...
  cacheTag(assistantSuggestionCacheTags.externalApi)
  cacheTag(assistantSuggestionCacheTags.user(userId))
...
export function createAssistantSuggestionsGateway(userId: string): SuggestionGenerator {
  return {
