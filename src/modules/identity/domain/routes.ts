...
export const IDENTITY_ROUTES = {
  ...
  AUTH: ['/login', '/signup', '/auth/callback', '/auth/error'],
} as const
...
export function isAuthRoute(pathname: string): boolean {
  return IDENTITY_ROUTES.AUTH.some((route) => pathname.startsWith(route))
}
...
export function isProtectedRoute(pathname: string): boolean {
  return IDENTITY_ROUTES.PROTECTED.some((route) => pathname.startsWith(route))
}
